...
(def components {:primary-button {:render                 (fn [_ state][::button "a button"])

                                  [:button :style]        (fn [_ state]
                                                            (println "xecuting basic style")
                                                            (rs/style :primary-button {:button-active? (rc/ls :button-active?)
                                                                                                 :button-hover?  (rc/ls :button-hover?)}))
                                  ;[:button :event->state] (fn [_ state] [:hover :click])

                                  [:button :on-click]     (fn [_ state e] (println "primary button click"))
                                  [:button :on-key-up]     (fn [_ state e] )
